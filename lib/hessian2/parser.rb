  ...
  module Parser
    ...
    def parse_data(data, os, refs = [], crefs = [])
      ...
      when 0x48 # untyped map ('H')
        val = {}
        while data[0] != 'Z'
          val[parse_key(data, os)] = parse_data(data, os, refs, crefs)
        end
      ...
      when 0x4f # object instance ('O')
        obj = {}.tap do |o|
          crefs[parse_int(data, os)].each{|a| o[a] = parse_data(data, os, refs, crefs)}
        end
        refs << obj
        obj
      when 0x51 # reference to map/list/object - integer ('Q')
        refs[parse_int(data, os)]
      when 0x52 # utf-8 string non-final chunk ('R')
        cut_large_string(data, os)
      ...
      when 0x55 # variable-length list/vector ('U')
        ...
        val = []
        while data[0] != 'Z'
          val << parse_data(data, os, refs, crefs)
        end
        refs << val # store a value reference
        os += 1 #=> 'Z'
        val
      when 0x56 # fixed-length list/vector ('V')
        parse_string(data, os) #=> type
        val = []
        parse_int(data, os, refs, crefs).times do
          val << parse_data(data, os, refs, crefs)
        end
        refs << val # store a value reference
        val
    ...
    private
    def parse_key(data, os) # string, int, long
      c = data[os += 1].unpack('C')[0]
      case c
      when 0x00..0x1f # utf-8 string length 0-31
        cut_direct_string(data, os, c)
